...
// This file provides browser-compatible mocks for Tauri APIs
...
async function mockInvoke(cmd, args = {}) {
  ...
  switch (cmd) {
    ...
    case 'set_endianness': {
      if (args.endianness !== 'little' && args.endianness !== 'big') {
  ...
  writeText: async (text) => {
    if (typeof window !== 'undefined') {
      try {
        ...
        console.log('[Mock Clipboard] Write failed, storing in memory');
      }
    }
...
export { invoke as mockInvoke } from '@tauri-apps/api/core';
...
// Override exports in browser environment
if (typeof window !== 'undefined' && !window.__TAURI__) {
  // This is a browser environment without Tauri
  // The mock will be injected via add_init_script
}

export async function invoke(cmd, args) {
  // If running in Tauri, use the real invoke
  if (typeof window !== 'undefined' && window.__TAURI__) {
...
export { mockDialog as dialog };
export { mockClipboard as clipboard };
export { mockWindowApi as window };
export { mockEvent as event };
...
if (typeof window !== 'undefined') {
  ...
  window.__mock_init__ = initMockData;
  ...
  window.__test_loadFiles__ = async function(pkgPath, a2lPath) {
    ...
    try {
      ...
      if (a2lPath) {
        ...
        debug.a2lResult = result;
        debug.a2lVarsCount = mockState.a2lVariables.length;
      }
      console.log('[Test] Files loaded:', debug);
